...
const GalleryList = () => {
  ...
  return (
    <div className="md:p-6">
      <h2 className="text-xl md:text-3xl mb-6 text-blue-600">
        Gallery List
      </h2>

      <p className="text-sm mb-3 text-blue-600">
        <em>Number of images: {galleryData.length}</em>
      </p>
              ...
              >
                ...
                <td className="py-2 px-4">
                  <img
                    src={
                      item.driveFileId
                        ? `https://drive.google.com/thumbnail?id=${item.driveFileId}&sz=w500`
                        : item.imageUrl
                    }
                    alt={`Gallery ${index + 1}`}
                    className="w-[50px] h-[50px] md:w-32 md:h-20 object-cover rounded shadow-sm"
                    onError={(e) => {
                      // Fallback to the original URL if the thumbnail fails
                      if (
                        item.imageUrl &&
